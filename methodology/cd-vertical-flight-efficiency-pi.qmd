...
link-citations: true
slug: cd-vertical-flight-efficiency-pi
---
...
(ref:vfe-cd-figure-001) Different steps of the analysis.


```{r pli-bli, out.width=c('800px', '400px'), fig.cap='(ref:vfe-cd-figure-001)'}
knitr::include_graphics("vfe-cd-figure-001.png")
```
...
descent [@airbus-cost-index; @sam-thesis].
...
be seen in Figure \@ref(fig:vfe-cd-figure-002).

(ref:vfe-cd-figure-002) Rolling window for level segment detection.


```{r vfe-cd-figure-002, out.width=c('500px', '300px'), fig.cap ='(ref:vfe-cd-figure-002)'}
knitr::include_graphics("vfe-cd-figure-002.png")
```
...
The formulas used for the calculation of all these results are shown in Table \@ref(tab:formulas).

Table: (\#tab:formulas) Formulas used for the calculation of the results


|   Result                                                     |    Formula                      |
|--------------------------------------------------------------|:--------------------------------|
| Total distance flown by flight $f$ during the climb/descent  | (ref:total-flown-cd)            |
| Percentage of distance flown level during the climb/descent  | (ref:perc-flown-level-cd)       |
| Average value of the distance flown level per flight         | (ref:avg-flown-flight-level)    |
| Median value of the level distance                           | (ref:median-level-dist)         |
| Total time flown by flight $f$ during the climb/descent      | (ref:total-flown-flight-cd)     |
| Percentage of time flown level during the climb/descent      | (ref:perc-flown-flight-level)   |
| Average time flown level per flight                          | (ref:avg-time-flight-level)     |
| Median value of the level time                               | (ref:median-time)               |
| Average number of level segments per flight                  | (ref:avg-level-segs)            |
| Median value of the number of level segments                 | (ref:median-level-segs)         |
| Percentage of flights considered as CCO or CDO               | $$P = \frac{n_0}{n} \cdot 100$$ |


(ref:total-flown-cd) $$D_f^+ = \begin{cases} D_{3000, f} - D_{\textrm{TOC} - \textrm{CCO}, f}, & \text{climbs} \\ D_{\textrm{TOD} - \textrm{CDO}, f} - D_{1800, f}, & \text{descents} \end{cases}$$

(ref:perc-flown-level-cd) $$D_{\textrm{perc}} = \frac{ \sum_{f} D_f}{\sum_{f} D_f^+} \cdot 100$$

(ref:avg-flown-flight-level) $$D_{\textrm{avg}} = \frac{ \sum_{f} D_f}{n}$$

(ref:median-level-dist) $$D_{med} = \begin{cases} D_m & \text{$n$ is odd, $m = (n+1)/2$} \\ \frac{D_m + D_{m+1}}{2}, & \text{$n$ is even, $m = n/2$} \end{cases}$$


(ref:total-flown-flight-cd) $$T_f^+ = \begin{cases} T_{3000, f} - T_{\textrm{TOC} - \textrm{CCO}, f}, & \text{climbs} \\ T_{\textrm{TOD} - \textrm{CDO}, f} - T_{1800, f}, & \text{descents} \end{cases}$$

(ref:perc-flown-flight-level) $$T_{\textrm{perc}} = \frac{\sum_f T_f}{\sum_f T_f^+}$$

(ref:avg-time-flight-level) $$T_{\textrm{avg}} = \frac{\sum_f T_f}{n}$$

(ref:median-time) $$T_{med} = \begin{cases} T_m & \text{$n$ is odd, $m = (n+1)/2$} \\ \frac{T_m + T_{m+1}}{2}, & \text{$n$ is even, $m = n/2$} \end{cases}$$

(ref:avg-level-segs) $$_{\textrm{avg}} = \frac{\sum_f L_f}{n}$$

(ref:median-level-segs) $$L_{med} = \begin{cases} \displaystyle L_m & \text{$n$ is odd, $m = (n+1)/2$} \\ \frac{L_m + L_{m+1}}{2}, & \text{$n$ is even, $m = n/2$} \end{cases}$$
...
[cpf]: /acronym/cpf/ "CPF"
[prisme]: /acronym/prisme/ "PRISME"

## Further Information

For further information please refer also to
[@cco-cdo-leaflet; @icns-vfe; @vertical-flight-efficiency-during-climb-and-descent_consultation].
