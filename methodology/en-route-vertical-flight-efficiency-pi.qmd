...
link-citations: true
slug: en-route-vertical-flight-efficiency-pi
---
...
calculated. The process is visualised in Figure \@ref(fig:vfe-er-figure-001).

(ref:vfe-er-figure-001) Different steps of the analysis.

```{r vfe-er-figure-001, out.width=c('800px', '400px'), fig.cap ='(ref:vfe-er-figure-001)'}
knitr::include_graphics("vfe-er-figure-001.png")
```
...
airport pair allows for plotting a diagram as in the Figure \@ref(fig:vfe-er-figure-002).

Figure \@ref(fig:vfe-er-figure-002) shows the histograms of the maximum filed altitudes both for the reference flights and the
...
(ref:vfe-er-figure-002) Example distribution of maximum altitudes.

```{r vfe-er-figure-002, out.width=c('900px', '900px'), fig.cap ='(ref:vfe-er-figure-002)'}
knitr::include_graphics("vfe-er-figure-002.png")
```



The data in Figure \@ref(fig:vfe-er-figure-002) above can be represented differently,
using percentiles as shown in Figure \@ref(fig:vfe-er-figure-003).
To obtain this, the percentages from Figure \@ref(fig:vfe-er-figure-002) are simply put successively. E.g. the percentages of
the chosen airport pair (blue bars: 2.6% at FL310, 21.1% at FL330 and 76.2% at FL350) are plotted at
their respective altitudes to get the blue line in Figure \@ref(fig:vfe-er-figure-003).
The same is done for the reference.
...
inefficiency (VFI) in order to flag only the inefficiencies.
The red shaded area in Figure \@ref(fig:vfe-er-figure-003)
...
(ref:vfe-er-figure-003) Example distribution of maximum altitudes.

```{r vfe-er-figure-003, out.width=c('900px', '900px'), fig.cap ='(ref:vfe-er-figure-003)'}
knitr::include_graphics("vfe-er-figure-003.png")
```


### Calculation of the metrics

The VFI is calculated using the percentiles as shown in Figure \@ref(fig:vfe-er-figure-003).
For each percentile range, the
...
Table \@ref(tab:formulas) summarises the formulas used for the calculation of the VFI metrics.
...
| Result                                                   |  Formula         |
|----------------------------------------------------------|------------------|
| Total vertical flight inefficiency (Total VFI)           | (ref:total-vfi)  |
| Vertical flight inefficiency per flight (VFI per flight) | (ref:vfi-flight) |

(ref:total-vfi) $$VFI_{\mathrm{tot}} = \sum_{i} \max(L_{\mathrm{ref}, i} - L_{\mathrm{act}, i}, 0) \cdot n_{\mathrm{act}, i}$$

(ref:vfi-flight) $$\frac{VFI_{\mathrm{tot}} }{\sum_{i} n_{\textrm{act}, i}}$$
...
[gcd]: /acronym/gcd/ "GCD"
[ftfm]: /acronym/ftfm/ "FTFM"


## Further Information

For further information please refer also to
[@icns-vfe; @en-route-vertical-flight-efficiency_consultation].
